...
const GOAL_AI_TIMEOUT_MS = 100000;
...
function normalizeChatState(inputState) {
  ...
  const roles = rawRoles.length
    ? rawRoles.map(normalizeRoleOnlyOnLoad)
...
function normalizeRoleOnlyOnLoad(role) {
...
function updateGoalChainLayer(role, phase, patch) {
  const baseGoalChain = createGoalChainState(role?.goalChain);
  const baseLayer = normalizeGoalLayerState(baseGoalChain?.[phase]);
  return createGoalChainState({
    ...baseGoalChain,
    [phase]: {
      ...baseLayer,
      ...patch,
...
- 如果无法生成合格目标，输出空字符串`;
  ...
  try {
    ...
    const response = await withTimeout(fetch(url, { method: 'POST', headers, body }), GOAL_AI_TIMEOUT_MS, '外部 AI 请求超时');
    if (!response.ok) {
...
async function tryGenerateGoalWithAI(role, ids, phase) {
  ...
  // (ctx is already initialized above)
  ...
  try {
    const response = await withTimeout(ctx.generateQuietPrompt({ quietPrompt: prompt, trimToSentence: false, removeReasoning: true }), GOAL_AI_TIMEOUT_MS, '内置 AI 请求超时');
...
async function assignGoalToPhase(role, ids, phase, force = false) {
  ...
  const task = withTimeout((async () => {
    ...
    const nextGoalChain = updateGoalChainLayer(role, phase, {
      goal: nextGoal,
      completed: false,
      progressSignals: 0,
    ...
    return nextRole;
  })(), GOAL_AI_TIMEOUT_MS, `目标生成超时: ${taskKey}`);

  const entry = setGoalAiTask(taskKey, task);
  try {
    return await task;
  } catch (error) {
...
async function ensureRoleInitialized(role) {
  const ids = getCurrentIdentifiers();
  let nextRole = normalizeRoleOnlyOnLoad(role);
...
function buildPrompt(state = ensureState(), settings = getSettings()) {
  ...
  const limitedRoles = enabledRoles.slice(0, MAX_ROLES_IN_PROMPT);
  const roleBlocks = [];
  ...
  const promptParts = [
    '[NPC 多角色自主性导演指令]',
    `你正在为同一轮主回复同时维护 ${limitedRoles.length} 个受控角色的连续行动与渐进变化。`,
    enabledRoles.length > limitedRoles.length ? `注意：为控制提示词体积，本轮只注入前 ${MAX_ROLES_IN_PROMPT} 个启用角色。` : '',
    '正文写作要求：',
  ...
  if (promptText.length > MAX_PROMPT_CHARS) {
    promptText = promptText.slice(0, MAX_PROMPT_CHARS).trim();
  }
  return promptText;
}
...
async function updateSelectedRoleField(key, rawValue) {
  ...
  if (key.startsWith('goalTemplates.')) {
    ...
    nextRole.goalChain = updateGoalChainLayer(selectedRole, 'early', {
